"""
Поиск ФИО в OCR-тексте PDF.

ЕДИНАЯ ТОЧКА ВХОДА:
    search_in_text(words_with_coords, search_terms)

Аргументы:
    words_with_coords — список словарей:
        {"text": str, "page": int, "x0": float, "y0": float, "x1": float, "y1": float}
    search_terms — строка через запятую или список строк

Возвращает список словарей:
    search_term, found_text, page, x0, y0, x1, y1
"""

import re
import time

# -----------------------------------------------------------------------------
# Настройки
# -----------------------------------------------------------------------------
MAX_GAP = 6
MAX_VERTICAL_DIST = 80
MAX_HORIZONTAL_DIST = 400
MAX_VERTICAL_DIST_V = 50   # для вертикального поиска инициалов в таблицах
COL_TOLERANCE = 35          # допуск по x для «той же колонки»


# =============================================================================
# НОРМАЛИЗАЦИЯ
# =============================================================================

try:
    import threading as _threading
    from pymorphy3 import MorphAnalyzer as _MorphAnalyzer

    _morph = _MorphAnalyzer()
    _morph_lock = _threading.Lock()
    _norm_cache: dict = {}
    _forms_cache: dict = {}
    _USE_PYMORPHY = True
    print("[search_fio] Используется pymorphy3")
except ImportError:
    _USE_PYMORPHY = False
    print("[search_fio] Используются встроенные правила")

_RULES = [
    ("СКОГО", "СКИЙ"), ("СКОМУ", "СКИЙ"), ("СКИМ", "СКИЙ"),
    ("ЦКОГО", "ЦКИЙ"), ("ЦКОМУ", "ЦКИЙ"), ("ЦКИМ", "ЦКИЙ"),
    ("СКОЙ", "СКИЙ"),  ("ЦКОЙ", "ЦКИЙ"),
    ("ОВЫМ", "ОВ"),    ("ЕВЫМ", "ЕВ"),
    ("ОВОМ", "ОВ"),    ("ЕВОМ", "ЕВ"),
    ("ОВОГО", "ОВ"),   ("ЕВОГО", "ЕВ"),
    ("ОВОМУ", "ОВ"),   ("ЕВОМУ", "ЕВ"),
    ("ОВЫХ", "ОВ"),    ("ЕВЫХ", "ЕВ"),
    ("ОВОЙ", "ОВ"),    ("ЕВОЙ", "ЕВ"),
    ("ОВУ", "ОВ"),     ("ЕВУ", "ЕВ"),
    ("ОВЕ", "ОВ"),     ("ЕВЕ", "ЕВ"),
    ("ОВЫ", "ОВ"),     ("ЕВЫ", "ЕВ"),
    ("ОВА", "ОВ"),     ("ЕВА", "ЕВ"),
    ("ИНЫМ", "ИН"),    ("ИНОГО", "ИН"),   ("ИНОМУ", "ИН"),
    ("ИНЫХ", "ИН"),    ("ИНОЙ", "ИН"),
    ("ИНУ", "ИН"),     ("ИНЕ", "ИН"),
    ("ИНЫ", "ИН"),     ("ИНА", "ИН"),
]


def normalize_surname(word: str) -> str:
    word = word.strip()
    if not word:
        return ""
    key = word.upper()

    if _USE_PYMORPHY:
        if key not in _norm_cache:
            with _morph_lock:
                parsed = _morph.parse(word.lower())
            for p in parsed:
                tag = str(p.tag)
                if "Surn" in tag and "masc" in tag:
                    _norm_cache[key] = p.normal_form.upper()
                    break
            else:
                for p in parsed:
                    if "Surn" in str(p.tag):
                        _norm_cache[key] = p.normal_form.upper()
                        break
                else:
                    _norm_cache[key] = parsed[0].normal_form.upper()
        return _norm_cache[key]

    w = key
    for suffix, replacement in _RULES:
        if w.endswith(suffix) and len(w) - len(suffix) >= 3:
            return w[: -len(suffix)] + replacement
    return w


def _all_surname_forms(word: str) -> frozenset:
    word = word.strip()
    if not word:
        return frozenset()
    key = word.upper()
    if not _USE_PYMORPHY:
        return frozenset([normalize_surname(word)])
    if key not in _forms_cache:
        with _morph_lock:
            parsed = _morph.parse(word.lower())
        _forms_cache[key] = frozenset(p.normal_form.upper() for p in parsed)
    return _forms_cache[key]


def _get_all_word_forms(word: str) -> frozenset:
    """Возвращает все возможные формы слова."""
    word = word.strip()
    if not word:
        return frozenset()
    key = word.upper()

    if not _USE_PYMORPHY:
        forms = {key}
        if key[-1] not in "АЕИОУЫЭЮЯ":
            for ending in ["А", "У", "ОМ", "Е"]:
                forms.add(key + ending)
            forms.add(key + "А")
        elif key.endswith(("А", "Я")):
            base = key[:-1]
            for ending in ["Ы", "Е", "У", "ОЙ", "ОЮ"]:
                forms.add(base + ending)
        return frozenset(forms)

    cache_key = "FORMS:" + key
    if cache_key not in _forms_cache:
        with _morph_lock:
            parsed = _morph.parse(word.lower())
        all_forms = set()
        for p in parsed:
            for form in p.lexeme:
                all_forms.add(form.word.upper())
        # Для слов, которых нет в словаре pymorphy3 (редкие имена, топонимы),
        # добавляем само слово как fallback — чтобы прямое совпадение работало
        if not all_forms:
            all_forms = {key}
        _forms_cache[cache_key] = frozenset(all_forms)

    return _forms_cache[cache_key]


# =============================================================================
# КЛАССИФИКАЦИЯ ТОКЕНОВ
# =============================================================================

def _is_initial(text: str) -> bool:
    return bool(re.fullmatch(r"[А-ЯЁ]\.?", text.upper()))


def _is_double_initial(text: str) -> bool:
    return bool(re.fullmatch(r"[А-ЯЁ]\.?[А-ЯЁ]\.?", text.upper()))


def _split_double_initial(text: str) -> list:
    return re.findall(r"[А-ЯЁ]", text.upper())


def _is_word(text: str) -> bool:
    return bool(re.fullmatch(r"[А-ЯЁа-яё]+(?:-[А-ЯЁа-яё]+)?", text))


# =============================================================================
# РАЗБИВКА СЛИТНЫХ ТОКЕНОВ
# =============================================================================

_MERGED_PATTERN = re.compile(
    r"^(?P<pre>(?:[А-ЯЁ]\.){1,2})?(?P<word>[А-ЯЁа-яё][-А-ЯЁа-яё]*)(?P<post>(?:[А-ЯЁ]\.){1,2})?$"
)


def _split_merged_token(text: str) -> list[str]:
    """
    Разбивает слитные токены вида 'Т.В.Соколова', 'СоколоваТ.В.', 'Т.Соколова'.
    Возвращает список подтокенов или [text] если разбивать не нужно.
    """
    m = _MERGED_PATTERN.match(text)
    if not m:
        return [text]

    pre  = m.group("pre")
    word = m.group("word")
    post = m.group("post")

    if not pre and not post:
        return [text]

    parts: list[str] = []
    if pre:
        for letter in re.findall(r"[А-ЯЁ]", pre):
            parts.append(letter + ".")
    parts.append(word)
    if post:
        for letter in re.findall(r"[А-ЯЁ]", post):
            parts.append(letter + ".")

    return parts if len(parts) > 1 else [text]


def _distribute_coords(base: dict, parts: list[str]) -> list[dict]:
    """
    Равномерно делит координаты исходного токена между подтокенами.
    """
    total_chars = sum(len(p) for p in parts)
    if total_chars == 0:
        return []

    result = []
    x_cursor = base["x0"]
    full_width = base["x1"] - base["x0"]

    for p in parts:
        ratio = len(p) / total_chars
        x1 = x_cursor + full_width * ratio
        result.append({
            **base,
            "text": p,
            "raw":  p,
            "x0":   x_cursor,
            "x1":   x1,
        })
        x_cursor = x1

    return result


# =============================================================================
# ПАРСИНГ ЗАПРОСА
# =============================================================================

def parse_query(query: str) -> dict:
    # Нормализуем «склеенные» инициалы: «А.Б.» → «А. Б.»
    query = re.sub(r"([А-ЯЁа-яё]\.)([А-ЯЁа-яё]\.?)", r"\1 \2", query)
    parts = re.findall(r"[А-ЯЁа-яё]+(?:-[А-ЯЁа-яё]+)?|[А-ЯЁа-яё]\.", query)

    words: list[str] = []
    initials: list[str] = []

    for p in parts:
        if _is_initial(p):
            initials.append(p[0].upper())
        else:
            words.append(p.upper())

    surname    = words[0] if words else None
    name_full  = words[1] if len(words) >= 2 else None
    patr_full  = words[2] if len(words) >= 3 else None

    name_initial = name_full[0] if name_full else (initials[0] if initials else None)
    patr_initial = (
        patr_full[0] if patr_full
        else (initials[1] if len(initials) >= 2 else None)
    )

    return {
        "surname":           surname,
        "surname_norm":      normalize_surname(surname) if surname else None,
        "name_initial":      name_initial,
        "name_full":         name_full,
        "patronymic_initial": patr_initial,
        "patronymic_full":   patr_full,
    }


# =============================================================================
# ПОДГОТОВКА ТОКЕНОВ
# =============================================================================

def _strip_numbering(text: str) -> tuple[str, int]:
    m = re.match(r"^\d+[.)\s]+", text)
    if m:
        return text[m.end():], m.end()
    # OCR иногда вставляет лишние символы между цифрой и скобкой: «7Т)», «1О)»
    m = re.match(r"^\d+[A-Za-zА-ЯЁа-яёA-Z]{0,2}[.)\s]+", text)
    if m:
        return text[m.end():], m.end()
    m = re.match(r"^[ЗзБбОоЧч][.)]\s*", text)
    if m:
        return text[m.end():], m.end()
    return text, 0


def _strip_punctuation(text: str) -> str:
    return re.sub(r"^[^\wА-Яа-яA-Za-z]+|[^\wА-Яа-яA-Za-z]+$", "", text)


def prepare_tokens(words_with_coords: list) -> list:
    raw_tokens: list[dict] = []

    for i, w in enumerate(words_with_coords):
        original = w["text"].strip()
        if not original:
            continue

        cleaned, prefix_len = _strip_numbering(original)
        if not cleaned:
            continue

        ends_with_dash = cleaned.endswith("-")
        cleaned = _strip_punctuation(cleaned)
        if not cleaned:
            continue

        if ends_with_dash and not cleaned.endswith("-"):
            cleaned += "-"

        if prefix_len:
            char_width = (w["x1"] - w["x0"]) / max(len(original), 1)
            new_x0 = w["x0"] + char_width * prefix_len
        else:
            new_x0 = w["x0"]

        raw_tokens.append({
            "text": cleaned,
            "page": w["page"],
            "x0":   new_x0,
            "y0":   w["y0"],
            "x1":   w["x1"],
            "y1":   w["y1"],
            "idx":  i,
        })

    # -------------------------------------------------------------------------
    # Склейка переносов: «Ива-» + «нов» → «Иванов»
    # Но: «Анай-» + «оол» на одной строке → «Анай-оол» (дефис сохраняется)
    # -------------------------------------------------------------------------
    merged: list[dict] = []
    skip_next = False

    for k, rw in enumerate(raw_tokens):
        if skip_next:
            skip_next = False
            continue
        text = rw["text"]
        if not text.endswith("-"):
            merged.append(rw)
            continue

        found_continuation = False
        for j in range(k + 1, min(k + 5, len(raw_tokens))):
            nxt = raw_tokens[j]
            if not _is_word(nxt["text"]):
                continue
            if not _is_word(text[:-1]):
                break

            y_diff      = nxt["y0"] - rw["y0"]
            line_height = rw["y1"] - rw["y0"]
            is_horizontal = j == k + 1
            is_vertical   = (
                y_diff > 0
                and y_diff < line_height * 3
                and abs(nxt["x0"] - rw["x0"]) < 100
            )

            if is_horizontal or is_vertical:
                # ИСПРАВЛЕНИЕ: если токены на одной строке — это составное слово
                # (например «Анай-оол»), дефис сохраняем.
                # Если на разных строках — это перенос, дефис убираем.
                is_line_break = y_diff > line_height * 0.5
                separator = "" if is_line_break else "-"

                merged.append({
                    **rw,
                    "text": text[:-1] + separator + nxt["text"],
                    "x1":   max(rw["x1"], nxt["x1"]),
                    "y1":   max(rw["y1"], nxt["y1"]),
                    "parts": [
                        {"x0": rw["x0"],  "y0": rw["y0"],  "x1": rw["x1"],  "y1": rw["y1"]},
                        {"x0": nxt["x0"], "y0": nxt["y0"], "x1": nxt["x1"], "y1": nxt["y1"]},
                    ],
                })
                skip_next = (j == k + 1)
                found_continuation = True
                break

            if y_diff > line_height * 3:
                break

        if not found_continuation:
            merged.append(rw)

    # -------------------------------------------------------------------------
    # Классификация + разбивка слитных токенов
    # -------------------------------------------------------------------------
    tokens: list[dict] = []

    for rw in merged:
        text = rw["text"]
        if not text:
            continue

        base = {k: rw[k] for k in ("page", "x0", "y0", "x1", "y1", "idx")}
        if "parts" in rw:
            base["parts"] = rw["parts"]

        # Двойные инициалы: «АБ», «А.Б.»
        if _is_double_initial(text):
            for letter in _split_double_initial(text):
                tokens.append({**base, "type": "initial", "text": letter, "raw": letter})
            continue

        # Одиночный инициал
        if _is_initial(text):
            tokens.append({**base, "type": "initial", "text": text.upper(), "raw": text})
            continue

        # Разбивка слитных «Т.В.Соколова»
        sub_parts = _split_merged_token(text)
        if len(sub_parts) > 1:
            for sub_tok in _distribute_coords({**base, "raw": text}, sub_parts):
                sub_text = sub_tok["text"]
                if _is_initial(sub_text):
                    tokens.append({**sub_tok, "type": "initial", "text": sub_text[0].upper()})
                elif _is_double_initial(sub_text):
                    for letter in _split_double_initial(sub_text):
                        tokens.append({**sub_tok, "type": "initial", "text": letter, "raw": letter})
                elif _is_word(sub_text):
                    tokens.append({**sub_tok, "type": "word", "text": sub_text.upper()})
                else:
                    tokens.append({**sub_tok, "type": "junk", "text": sub_text.upper()})
            continue

        # Обычное слово
        if _is_word(text):
            tokens.append({**base, "type": "word", "text": text.upper(), "raw": text})
        else:
            tokens.append({**base, "type": "junk", "text": text.upper(), "raw": text})

    return tokens


# =============================================================================
# СОПОСТАВЛЕНИЕ
# =============================================================================

def _surname_matches(token: dict, query: dict) -> bool:
    if not query["surname_norm"] or not token["text"]:
        return False
    # Прямое сравнение (важно для редких имён/фамилий с дефисом,
    # которых нет в словаре pymorphy3)
    if token["text"].upper() == query["surname"].upper():
        return True
    if normalize_surname(token["text"]) == query["surname_norm"]:
        return True
    token_forms = _all_surname_forms(token["text"])
    query_forms = _all_surname_forms(query["surname"])
    if token_forms & query_forms:
        return True
    all_token_forms = _get_all_word_forms(token["text"])
    all_query_forms = _get_all_word_forms(query["surname"])
    return bool(all_token_forms & all_query_forms)


def _name_matches(token: dict, initial, full) -> bool:
    if initial is None or not token["text"]:
        return False
    if token["text"][0].upper() != initial.upper():
        return False
    if token["type"] == "initial":
        return True
    if token["type"] == "word":
        # Слово принимаем только если знаем полное имя для сверки
        if full is not None:
            return normalize_surname(token["text"]) == normalize_surname(full)
        return False
    return False


# =============================================================================
# ПРОСТРАНСТВЕННАЯ БЛИЗОСТЬ
# =============================================================================

def _tokens_are_close(anchor: dict, candidate: dict) -> bool:
    """Горизонтальная близость — токены на одной строке."""
    if abs(anchor["y0"] - candidate["y0"]) > MAX_VERTICAL_DIST:
        return False
    gap = max(anchor["x0"], candidate["x0"]) - min(anchor["x1"], candidate["x1"])
    return gap <= MAX_HORIZONTAL_DIST


def _same_column(anchor: dict, candidate: dict) -> bool:
    """Вертикальная близость — токен в той же колонке таблицы."""
    anchor_cx    = (anchor["x0"] + anchor["x1"]) / 2
    candidate_cx = (candidate["x0"] + candidate["x1"]) / 2
    return abs(anchor_cx - candidate_cx) <= COL_TOLERANCE


# =============================================================================
# ПОИСК ИНИЦИАЛОВ В ОКНЕ (горизонтальный)
# =============================================================================

def _find_initials_in_window(tokens, start, direction, anchor, query):
    n = len(tokens)
    name_tok = None
    patr_tok = None
    matched   = []
    gap       = 0
    j         = start

    anchor_line_height = anchor["y1"] - anchor["y0"]
    y_tolerance        = max(anchor_line_height * 2.5, 20)

    while 0 <= j < n and gap <= MAX_GAP:
        t = tokens[j]

        page_diff = t["page"] - anchor["page"]
        if abs(page_diff) > 1:
            break

        if page_diff == 0:
            y_diff = abs(t["y0"] - anchor["y0"])
            if y_diff > y_tolerance:
                break
            if not _tokens_are_close(anchor, t):
                break
            # Если токен на другой строке — он должен быть
            # в той же горизонтальной зоне (не левее фамилии)
            if y_diff > anchor_line_height and t["x0"] < anchor["x0"] - 10:
                break

        if t["type"] in ("word", "initial"):
            if name_tok is None and _name_matches(t, query["name_initial"], query["name_full"]):
                name_tok = t
                matched.append(t)
            elif patr_tok is None and _name_matches(t, query["patronymic_initial"], query["patronymic_full"]):
                patr_tok = t
                matched.append(t)
            else:
                break
        else:
            gap += 1

        j += direction

    return name_tok, patr_tok, matched


# =============================================================================
# ПОИСК ИНИЦИАЛОВ ПО ВЕРТИКАЛИ (таблицы)
# =============================================================================

def _find_initials_vertically(tokens: list, surname_tok: dict, query: dict):
    page = surname_tok["page"]
    sy0  = surname_tok["y0"]
    sy1  = surname_tok["y1"]

    needs_name = query["name_initial"] is not None
    needs_patr = query["patronymic_initial"] is not None

    below = [
        t for t in tokens
        if t["page"] == page
        and t["type"] in ("initial", "word")
        and t["y0"] > sy1
        and t["y0"] < sy0 + MAX_VERTICAL_DIST_V
        and _same_column(surname_tok, t)
    ]
    above = [
        t for t in tokens
        if t["page"] == page
        and t["type"] in ("initial", "word")
        and t["y1"] < sy0
        and t["y1"] > sy0 - MAX_VERTICAL_DIST_V
        and _same_column(surname_tok, t)
    ]

    below.sort(key=lambda t: t["y0"])
    above.sort(key=lambda t: t["y0"], reverse=True)

    name_tok = None
    patr_tok = None
    matched   = []

    for group in (below, above):
        for t in group:
            if not t["text"]:
                continue
            if name_tok is None and needs_name and _name_matches(t, query["name_initial"], query["name_full"]):
                name_tok = t
                matched.append(t)
            elif patr_tok is None and needs_patr and _name_matches(t, query["patronymic_initial"], query["patronymic_full"]):
                patr_tok = t
                matched.append(t)
        if matched:
            break

    return name_tok, patr_tok, matched


# =============================================================================
# ОСНОВНОЙ ПОИСК
# =============================================================================

def _search_by_surname_only(tokens: list, query: dict) -> list:
    results = []
    seen    = set()
    search_forms = _get_all_word_forms(query["surname"])

    for tok in tokens:
        if tok["type"] != "word" or not tok["text"]:
            continue

        token_in_forms      = tok["text"].upper() in search_forms
        token_forms         = _get_all_word_forms(tok["text"])
        query_in_token_forms = query["surname"].upper() in token_forms

        if not (token_in_forms or query_in_token_forms):
            continue

        key = (tok["page"], round(tok["x0"], 1), round(tok["y0"], 1))
        if key in seen:
            continue
        seen.add(key)
        results.append({
            "search_term": query.get("_raw", ""),
            "found_text":  tok.get("raw", tok["text"]),
            "page":        tok["page"],
            "x0":          tok["x0"],
            "y0":          tok["y0"],
            "x1":          tok["x1"],
            "y1":          tok["y1"],
        })

    return results


def _search_by_initials_only(tokens: list, query: dict) -> list:
    results     = []
    seen        = set()
    needs_name  = query["name_initial"] is not None
    needs_patr  = query["patronymic_initial"] is not None
    n           = len(tokens)

    for i, tok in enumerate(tokens):
        if tok["type"] != "initial" or not tok["text"]:
            continue

        name_tok = None
        patr_tok = None
        matched  = []

        if needs_name and tok["text"][0] == query["name_initial"]:
            name_tok = tok
            matched.append(tok)

            if needs_patr:
                for j in range(i + 1, min(i + 1 + MAX_GAP, n)):
                    t = tokens[j]
                    if not t["text"] or t["type"] == "junk":
                        continue
                    if t["type"] != "initial":
                        break
                    if not _tokens_are_close(tok, t):
                        break
                    if t["text"][0] == query["patronymic_initial"]:
                        patr_tok = t
                        matched.append(t)
                        break

        elif not needs_name and needs_patr and tok["text"][0] == query["patronymic_initial"]:
            patr_tok = tok
            matched.append(tok)

        name_ok = (not needs_name) or (name_tok is not None)
        patr_ok = (not needs_patr) or (patr_tok is not None)
        if not (name_ok and patr_ok):
            continue

        for m in matched:
            key = (m["page"], round(m["x0"], 1), round(m["y0"], 1))
            if key in seen:
                continue
            seen.add(key)
            results.append({
                "search_term": query.get("_raw", ""),
                "found_text":  m.get("raw", m["text"]),
                "page":        m["page"],
                "x0":          m["x0"],
                "y0":          m["y0"],
                "x1":          m["x1"],
                "y1":          m["y1"],
            })

    return results


def _append_result(results, seen, query, m):
    """Вспомогательная функция: добавляет токен в результаты (с учётом parts)."""
    if "parts" in m:
        for part_idx, part in enumerate(m["parts"]):
            key = (m["page"], round(part["x0"], 1), round(part["y0"], 1))
            if key in seen:
                continue
            seen.add(key)
            results.append({
                "search_term": query.get("_raw", ""),
                "found_text":  m.get("raw", m["text"]) if part_idx == 0 else "",
                "page":        m["page"],
                "x0":          part["x0"],
                "y0":          part["y0"],
                "x1":          part["x1"],
                "y1":          part["y1"],
            })
    else:
        key = (m["page"], round(m["x0"], 1), round(m["y0"], 1))
        if key in seen:
            return
        seen.add(key)
        results.append({
            "search_term": query.get("_raw", ""),
            "found_text":  m.get("raw", m["text"]),
            "page":        m["page"],
            "x0":          m["x0"],
            "y0":          m["y0"],
            "x1":          m["x1"],
            "y1":          m["y1"],
        })


def _search_fio(tokens: list, query: dict) -> list:
    """Режим: фамилия + инициалы."""
    results = []
    seen    = set()
    needs_name = query["name_initial"] is not None
    needs_patr = query["patronymic_initial"] is not None

    for i, tok in enumerate(tokens):
        if tok["type"] != "word" or not tok["text"]:
            continue
        if not _surname_matches(tok, query):
            continue

        # Горизонтальный поиск инициалов (вперёд и назад)
        name_f, patr_f, match_f = _find_initials_in_window(tokens, i + 1, +1, tok, query)
        name_b, patr_b, match_b = _find_initials_in_window(tokens, i - 1, -1, tok, query)

        name_tok = name_f or name_b
        patr_tok = patr_f or patr_b
        matched  = [tok] + match_f + match_b

        # Вертикальный поиск (fallback для таблиц)
        if (needs_name and name_tok is None) or (needs_patr and patr_tok is None):
            vname, vpatr, vmatch = _find_initials_vertically(tokens, tok, query)
            if needs_name and name_tok is None and vname is not None:
                name_tok = vname
                matched.extend(vmatch)
            if needs_patr and patr_tok is None and vpatr is not None:
                patr_tok = vpatr
                for m in vmatch:
                    if m not in matched:
                        matched.append(m)

        if needs_name and name_tok is None:
            continue
        if needs_patr and patr_tok is None:
            continue

        # Если в запросе полные имя И отчество (не инициалы) —
        # требуем чтобы хотя бы одно из них нашлось как полное слово,
        # а не как инициал. Это отсекает ложные срабатывания вида:
        # запрос "Иванов Иван Иванович" → не должен находить "Иванов И.И."
        if query["name_full"] is not None and query["patronymic_full"] is not None:
            surname_forms = _get_all_word_forms(query["surname"])
            has_full_word = any(
                t["type"] == "word" and t["text"] not in surname_forms
                for t in matched if t is not tok
            )
            if not has_full_word:
                continue

        for m in matched:
            _append_result(results, seen, query, m)

    return results


# =============================================================================
# ЕДИНАЯ ТОЧКА ВХОДА
# =============================================================================

def search_in_text(words_with_coords: list, search_terms) -> list:
    """
    Ищет ФИО в OCR-тексте PDF.

    Аргументы:
        words_with_coords — список словарей:
            {"text": str, "page": int,
             "x0": float, "y0": float, "x1": float, "y1": float}
        search_terms — строка через запятую или список строк

    Возвращает список словарей:
        search_term, found_text, page, x0, y0, x1, y1
    """
    start = time.time()

    if isinstance(search_terms, str):
        terms = [t.strip() for t in search_terms.split(",") if t.strip()]
    else:
        terms = [t.strip() for t in search_terms if t.strip()]

    tokens = prepare_tokens(words_with_coords)
    found  = []

    for term in terms:
        query = parse_query(term)
        query["_raw"] = term

        has_surname  = query["surname"] is not None
        has_initials = (
            query["name_initial"] is not None
            or query["patronymic_initial"] is not None
        )

        if has_surname and has_initials:
            found.extend(_search_fio(tokens, query))
        elif has_surname:
            found.extend(_search_by_surname_only(tokens, query))
        elif has_initials:
            found.extend(_search_by_initials_only(tokens, query))

    print(
        f"[TIME] Search: {time.time() - start:.2f}s | "
        f"запросов: {len(terms)} | найдено: {len(found)}"
    )

    return found